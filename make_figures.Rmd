...
repression_labels <- tribble(
  ~x,   ~y,  ~text,
  1.5, 3.8, "(Less repression)",
  1.5, 0.2, "(More repression)"
)
...
fig_cso_repression <- ggplot(avg_cso_repression, 
  ...
  geom_text(data = repression_labels, aes(x = x, y = y, label = text),
            ...
            size = 2, color = "grey50", family = "Asap Condensed") +
...
fig_combined <- (fig_cso_repression + labs(tag = "A")) + 
  blank + 
  (fig_ngo_regulations + labs(tag = "B")) +
